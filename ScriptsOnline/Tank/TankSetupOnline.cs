...
public class TankSetupOnline : NetworkBehaviour 
{
    [Header("UI")]
    ...
    public override void OnStartClient()
    {
        ...
        if (!isServer) //if not hosting, we had the tank to the gamemanger for easy access!
            GameManagerOnline.AddTank(gameObject, m_PlayerNumber, m_Color, m_PlayerName, m_LocalID);
    ...
    public void Update()
    {
        ...
        if (GameManagerOnline.s_Instance.m_GameIsFinished && !m_IsReady)
        {
    ...
    {
        GameManagerOnline.s_Instance.RemoveTank(gameObject);
    }
}
