...
[Serializable]
public class TankManagerOnline
{
    // This class is to manage various settings on a tank.
    // It works with the GameManagerOnline class to control how the tanks behave
    ...
    [HideInInspector] GameObject m_Instance;             // A reference to the instance of the tank when it is created.
    ...
    [HideInInspector] public string m_PlayerName;                    // The player name set in the lobby
    [HideInInspector] public int m_LocalPlayerID;                    // The player localID (if there is more than 1 player on the same machine)

    public TankMovementOnline m_Movement;        // References to various objects for control during the different game phases.
    public TankShootingOnline m_Shooting;
    public TankHealthOnline m_Health;
    public TankSetupOnline m_Setup;

    public void Setup()
    {
        // Get references to the components.
        m_Movement = m_Instance.GetComponent<TankMovementOnline>();
        m_Shooting = m_Instance.GetComponent<TankShootingOnline>();
        m_Health = m_Instance.GetComponent<TankHealthOnline>();
        m_Setup = m_Instance.GetComponent<TankSetupOnline>();
